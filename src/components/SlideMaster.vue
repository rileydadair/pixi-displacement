...
export default {
  ...
  methods: {
    initWheel() {
      ...
      this.app.addEventListener('mousewheel', this.onWheel)
      this.app.addEventListener('wheel', this.onWheel)
      this.app.addEventListener('DOMMouseScroll', this.onWheel)
      this.app.addEventListener('MozMousePixelScroll', this.onWheel)
    },
    ...
    removeWheel() {
      this.app.removeEventListener('mousewheel', this.onWheel)
      this.app.removeEventListener('wheel', this.onWheel)
      this.app.removeEventListener('DOMMouseScroll', this.onWheel)
      this.app.removeEventListener('MozMousePixelScroll', this.onWheel)
    },
    ...
    onClick(e, index, direction = this.getDirection(this.currentIndex, index)) {
      ...
      if (this.canEvent && this.currentIndex !== index) this.setCurrentSlide(e, index, direction)
    },
    ...
    setCurrentSlide(e, index, direction) {
      ...
      this.onSliderEvent ? this.onSliderEvent(e, this, this.currentIndex, nextIndex, direction) :
        this.$emit('onSliderEvent', e, this, this.currentIndex, nextIndex, direction)

      // Wait to enable all events.
      if (this.eventTimeout) setTimeout(() => this.toggleEvents(), this.eventTimeout)
  ...
  render() {
    ...
    const slides = this.slideWrap ?
      this.$slots.slides.map((item, index) => (
        <div
          class={`slider__slide${this.currentIndex === index ? ' is-active' : ''}`}
    ...
    const controls = this.$slots.prevButton || this.$slots.nextButton ?
      (
        <div class="controls">
          <button
    ...
    const indicators = this.indicators ?
      (
        <ul class="indicators">
          {this.$slots.slides.map((item, index) => (
            <li
              class={`indicators__item${this.currentIndex === index ? ' is-active' : ''}`}
            ...
            >
              <button
                ...
                refInFor
              >
                {this.$slots.indicatorsList.length ? this.$slots.indicatorsList[index] : null}
              </button>
            </li>
          ))}
        </ul>
